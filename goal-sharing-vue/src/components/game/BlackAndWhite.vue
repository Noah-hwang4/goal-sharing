  ...
  <v-card>
    ...
    <v-list-item>
      <v-list-item-content>
        ...
        <div>
          <v-btn class="black" v-for="index in 9" :key="index-1" v-if="evenList.indexOf(index-1) > -1 && !p1['c' + (index-1)]" x-small style="min-width: 31px;">?</v-btn>
          <v-btn class="white" v-for="index in 9" :key="index-1" v-if="evenList.indexOf(index-1) === -1 && !p1['c' + (index-1)]" x-small style="min-width: 31px;">?</v-btn>
        </div>
      </v-list-item-content>
    </v-list-item>
    ...
    <v-list-item>
      <v-list-item-content>
        ...
        <div>
          ...
          <v-btn :style="main.c0.cStyle" v-if="main.c0.cShow" small>{{ main.c0.cValue }}</v-btn>
          <v-btn :style="main.c1.cStyle" v-if="main.c1.cShow" small>{{ main.c1.cValue }}</v-btn>
        </div>
      </v-list-item-content>
    </v-list-item>
    ...
    <v-list-item>
      <v-list-item-content>
        ...
        <div>
          <v-btn class="black" v-for="index in 9" :key="index-1" v-on:click="open(index-1)" v-if="evenList.indexOf(index-1) > -1 && !p2['c' + (index-1)]" x-small style="min-width: 31px;">{{ index-1 }}</v-btn>
          <v-btn class="white" v-for="index in 9" :key="index-1" v-on:click="open(index-1)" v-if="evenList.indexOf(index-1) === -1 && !p2['c' + (index-1)]" x-small style="min-width: 31px;">{{ index-1 }}</v-btn>
        </div>
      </v-list-item-content>
    </v-list-item>
  </v-card>
    ...
    methods: {
      ...
      setOpen: function (index, number) {
        ...
        if (this.evenList.indexOf(number) > -1) {
          this.main['c' + index].cStyle = 'min-width: 31px; background: black; color: white;'
        } else {
          this.main['c' + index].cStyle = 'min-width: 31px; background: white; color: black;'
        }
